    ...
    with _conn() as conn:
        with conn.cursor() as cur:
            ...
            cur.execute(
                ...
                BEGIN
                  IF current_setting('server_version_num')::INT >= 150000 THEN
                    ALTER VIEW public.ml_feedback_daily SET (security_invoker = true);
                  END IF;
                END
                $$;
                """
            ...
            cur.execute(
                ...
                BEGIN
                  IF current_setting('server_version_num')::INT >= 150000 THEN
                    ALTER VIEW public.ml_deck_outcomes SET (security_invoker = true);
                  END IF;
                END
                $$;
                """
            ...
            # - Revoke anon/authenticated table & view access.
            # - Keep service_role access for controlled backend usage.
            cur.execute("ALTER TABLE public.analysis_events ENABLE ROW LEVEL SECURITY")
            cur.execute("ALTER TABLE public.battle_feedback ENABLE ROW LEVEL SECURITY")
            cur.execute("ALTER TABLE public.online_calibration ENABLE ROW LEVEL SECURITY")
